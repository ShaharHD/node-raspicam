 ...
 *
 * @param {Object} opts Options: command, freq, delay, width, height, quality, encoding, filepath, filename, timeout
 */
function RaspiCam( opts ) {
  ...
  if(typeof opts.command === "undefined" || typeof opts.output === "undefined"){
    console.log("Error: RaspiCam: must define command and output");
    return false;
  }
  ...
  for(var opt in opts){
    if(opt.length <= 3){
      ...
      if(typeof parameters[opt] !== "undefined"){
        ...
        if (this.opts[parameters[opt]] !== opts[opt]) {
      ...
      if(typeof flags[opt] !== "undefined"){
        ...
        // reassign it to the full word
        if (this.opts[flags[opt]] !== opts[opt]) {
...
RaspiCam.prototype.defaultOpts = function(){
  ...
  this.opts.command = this.opts.command || 'photo';//photo, timelapse or video

  this.opts.width = this.opts.width || 2592;
  this.opts.height = this.opts.height || 1944;

  this.opts.log = this.opts.log || true;
...
RaspiCam.prototype.watchDirectory = function( ) {
  ...
  this.watcher = fs.watch(this.filepath, function(event, filename){
    ...
    if(event === "rename"){
      ...
      // if (this.opts.log){
        // console.log('raspicam::watcher::event ' + event);s
      // }
...
RaspiCam.prototype.start = function( ) {
  ...
  switch(this.opts.command){
    ...
    case 'video':
      ...
      break;
    default:
      this.emit("start", "Error: command must be photo, timelapse or video", new Date().getTime() );
      return false;
  }
